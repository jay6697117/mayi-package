			...
			<view class="l-chat-body " @tap="onChatClick">
				...
				:scroll-top="chatScrollTop">
					<view class="l-char-scroll-content">
						...
						:class="{ 'l-chat-mine': s.content.from_id === myImUsername }">
							<!-- 时间 -->
							...
							<view class="l-chat-item-content">
								...
								<view class="l-chat-avatar">
									...
									v-if="s.content.from_id == myImUsername"
								...
								<view class="l-chat-view">
									...
									<view class="l-chat-name">
										{{ s.content.from_name || s.content.from_id}}
									</view>
									<!-- 信息展示 text/img/ -->
...
<script>
	...
	export default{
		...
		computed:{
            ...
            myImUsername(){
                return this.$store.getters.jimUserInfo.username;
            },
			jimLocalhost(){
		...
		onLoad(e) {
            this.gid = e.gid || ''
			if(this.hasJMLogin){
				this.$store.dispatch('jimGetGroupInfo', this.gid);
			}
		},
		watch:{
			hasJMLogin(n){
		...
		methods:{
			...
			submit(){
				...
				let params = {
					...
					target_gname: this.singleInfo.name,
					target_gid: this.singleInfo.gid
				}
				this.$store.dispatch('jimSendGroupMsg', params);
				this.content = '';
			},
			previewImage(urls){
				uni.previewImage({
			...
			sendImageItem(tempFilePaths, info){
				...
				this.$store.dispatch('sendGroupPic', params);
				this.content = '';
				if(this.isUpload){
					this.isUpload = !this.isUpload
				}
