...
<script>
	...
	import { byInviteCode, applyCustomer } from '@/http/user.js'
	...
	methods: {
		...
		changeImage (list) {
			getQiniuToken().then(({ key, qiniu_uri, token }) => {
				...
				this.form.license.splice(this.form.license.length - 1, 1)
			})
		},
		onSubmit () {
			// uni.showToast({ title: "申请成功", icon: "none" });
			if(this.$isH5Share && this.userInfo.code) {
				byInviteCode({ phone: this.form.phone, invite_code: this.userInfo.code }).then(res => {
					console.log(res)
					this.form.invite_user_id = res.item.id
					this.submitting()
				});
			} else {
				this.submitting()
